...
export async function haalEnsemble(editie: Plek, model: string): Promise<EnsembleDaily> {
  const url = `https://ensemble-api.open-meteo.com/v1/ensemble?latitude=${editie.lat}&longitude=${editie.lon}&daily=${VARS}&models=${model}&timezone=Europe%2FAmsterdam&start_date=${editie.start}&end_date=${editie.end}`;
...
export async function haalHarmonie(editie: Plek): Promise<HarmonieTemp> {
  try {
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${editie.lat}&longitude=${editie.lon}&daily=temperature_2m_max,temperature_2m_min&models=knmi_harmonie_arome_netherlands&timezone=Europe%2FAmsterdam&forecast_days=3`;
...
export async function haalBias(editie: Plek): Promise<number> {
  try {
    const dag = (terug: number) => new Date(Date.now() - terug * 86_400_000).toISOString().slice(0, 10);
    const [prev, arch] = await Promise.all([
      fetch(`https://previous-runs-api.open-meteo.com/v1/forecast?latitude=${editie.lat}&longitude=${editie.lon}&start_date=${dag(9)}&end_date=${dag(3)}&hourly=temperature_2m_previous_day2&models=ecmwf_ifs025&timezone=Europe%2FAmsterdam`).then((r) => r.json()),
      fetch(`https://archive-api.open-meteo.com/v1/archive?latitude=${editie.lat}&longitude=${editie.lon}&start_date=${dag(9)}&end_date=${dag(3)}&daily=temperature_2m_max&timezone=Europe%2FAmsterdam`).then((r) => r.json()),
    ]);
    const fouten: number[] = [];
